...
export default function FeedbackPage() {
  ...
  const sendReaction = async (msgId, emoji) => {
    await fetch("http://localhost:8080/api/react", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message_id: msgId, user_id: currentUser, emoji }),
    });
  };
    ...
    <div>
      <nav style={{ background: "#eee", padding: "10px" }}>
        ...
        <button
          ...
          style={{ fontWeight: currentUser === 1 ? "bold" : "normal" }}
        >
          Alice
        </button>
        <button
          onClick={() => setCurrentUser(2)}
          style={{ fontWeight: currentUser === 2 ? "bold" : "normal" }}
        >
          Bob
        </button>
      </nav>
      ...
      <div className="message">
        <p>Hello World!</p>
        <button onClick={() => sendReaction(1, "😂")}>😂</button>
        <button onClick={() => sendReaction(1, "🔥")}>🔥</button>
      </div>
